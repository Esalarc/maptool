...
public class WebAppInitiative {
  ...
  private JsonObject getInitiativeDetails() {
    ...
    for (InitiativeList.TokenInitiative token : tokenInitList) {
      if (InitiativeListModel.isTokenVisible(token.getToken(), initiativeList.isHideNPC())) {
        ...
        tokJSon.addProperty("tokenType", token.getToken().getType().name());
        tokJSon.addProperty("visible", token.getToken().isVisible());
        ...
        for (String tokenState : tokenStates) {
          ...
          if (value != null && FunctionUtil.getBooleanValue(value)) {
            JsonObject tokState = new JsonObject();
            tokState.addProperty("state", tokenState);
            BooleanTokenOverlay overlay = tokenStatesMap.get(tokenState);
            if (overlay != null && overlay instanceof ImageTokenOverlay) {
              tokState.addProperty("hasImage", true);
              tokState.addProperty("image", ((ImageTokenOverlay) overlay).getAssetId().toString());
            } else {
              tokState.addProperty("hasImage", false);
            }
            states.add(tokState);
          }
        }
        tokJSon.add("activeStates", states);
  ...
  void processInitiativeMessage(JsonObject json) {
    ...
    if (!currentInit.equals(json.get("currentInitiative").getAsString())
        || !currentRound.equals(json.get("currentInitiative").getAsString())) {
